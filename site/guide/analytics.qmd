...
edc-core gives data managers **real SQL and real R, inside the EDC**, against
versioned, analysis-ready study datasets.
...
The R tab sends scripts to a sandboxed server-side R container (Rocker +
plumber). Each run executes in a fresh subprocess against the same pinned
snapshot, with two helpers in scope:
