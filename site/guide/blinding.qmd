...
`demo-admin` performing the break-the-blind action.
...
![](../images/blinded-entry.png){.screenshot fig-alt="The dosing form as the site coordinator sees it: the assigned arm is visible alongside the dose fields"}

The monitor opens the same form and sees the item's presence and entry
status, but never its value:

![](../images/blinded-field.png){.screenshot fig-alt="The same dosing form as the monitor sees it: the arm field is locked and shows a blinded placeholder instead of the value"}
...
- **Arm assignments from an RTSM should land on a blinded item.** The
  [RTSM integration](rtsm-integration.qmd) writes through a service
...
- **Blinded items are never codable**, and edit checks that reference
  ...
  [Rules and derivations](rules-and-derivations.qmd#pitfalls).
...
![](../images/break-blind.png){.screenshot fig-alt="The subject matrix with the break-the-blind confirmation open: a category selector and required reason before anything is recorded"}
...
[team page](user-admin.qmd#study-teams) shows who holds `data.unblind`,
...
- [RTSM integration](rtsm-integration.qmd): how the arm arrives without a
  human ever typing it.
- [Data capture](data-capture.qmd#blinded-items): the entry-side summary.
- [User administration](user-admin.qmd): granting and revoking
  `data.unblind`.
