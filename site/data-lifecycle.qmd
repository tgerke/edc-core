...
[amendments](guide/study-builds.qmd), with deliberate, audited migration of
existing subjects.
...
[queries](guide/review.qmd) when violated, and auto-close when the data is
...
import](guide/lab-import.qmd) (`item_value.imported`), or [RTSM
integration](guide/rtsm-integration.qmd) (`item_value.integrated`).
...
- **User account history** (§4.2.2(a)(i)): account creation, activation
  ...
  additionally lands in the [access log](guide/user-admin.qmd).
...
retention](deployment.qmd#access-logging-and-log-retention) for the
...
| Review | Where |
|---|---|
| Data review | [Query dashboard](guide/review.qmd): open/answered/closed lifecycle, manual and system-raised, monitor reopen |
| Audit trail review | `/studies/:id/audit` — filter by action, entity, actor, time; export CSV (`audit.review`-gated) |
| Access review | [Access log](guide/user-admin.qmd) with CSV export |
| Security events | [Anomaly review](deployment.qmd#security-anomaly-detection) with audited acknowledgement |
...
Inbound transfers ([lab CSV](guide/lab-import.qmd),
[RTSM](guide/rtsm-integration.qmd)) go through the same audited write path
...
[snapshots](guide/analytics.qmd): the same snapshot version re-reads
...
2. Complete [medical coding](guide/medical-coding.qmd) from the work queue.
...
Analysis-side, [workbench](guide/analytics.qmd) SQL and R executions record
their snapshot version, full code, logs, and outputs — data extraction is
documented by construction (§4.2.6(c)).
...
audit trail, and the [deployment controls](deployment.qmd) (TLS, encrypted
...
- **Blinding** (§4.1): item-level blinding is enforced at form reads,
  ...
  define map onto grants you can review — and the grant history documents
  who could see unblinded data, and when, for §4.1.4.
- **Computerised systems** (§4.3): validation evidence ships per release as
  the [validation pack](compliance.qmd); security controls, backup, and
  monitoring are the [deployment page](deployment.qmd). Training (§4.3.2)
...
[traceability matrix](compliance.qmd).
