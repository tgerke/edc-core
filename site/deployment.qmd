...
database password, no TLS, and every service port published to the host. It
is the reference topology, not a hardened one. This page is what to change
before clinical data touches the system — and how the pieces map to GDPR and
...
multi-hop setups) so the [access log](guide/user-admin.qmd), session
...
- [ ] `EDC_TRUST_PROXY` set to match your proxy topology — and never set
  without a proxy actually in front.
