...
The compose file in `infra/` is a **local development stack**: hard-coded
database password, no TLS, every service port published to the host, and a web
container that runs the Vite dev server. It is the reference topology, not a
hardened one. This page is what to change before clinical data touches the
system — and how the pieces map to GDPR and HIPAA hosting expectations.
...
The R engine mounts the lake read-only and keeps no state of its own. The web
container serves the SPA and holds nothing.
...
(API ↔ Postgres, API ↔ R engine) stays on the container network. The moment
...
The development compose file publishes Postgres (5432) and the R engine
(8000) to the host for convenience. In production, expose **only** the
reverse proxy. The R engine in particular executes analyst code by design;
it must never be reachable from outside the container network.
...
application-level cryptography — encrypt at the volume layer, where it is
operationally boring and covers everything at once:
...
is the evidence trail for access reviews; point your monitoring at repeated
401/403 patterns.
...
- [ ] Replace `POSTGRES_PASSWORD` / `DATABASE_URL` credentials (the compose
  default is labeled `edc-dev-only` for a reason) and inject them as secrets,
  not baked into files.
- [ ] `NODE_ENV=production` on the API container (secure cookies).
- [ ] TLS-terminating reverse proxy in front; no other published ports —
  in particular not 5432 (Postgres) or 8000 (R engine).
...
- [ ] Review the authentication knobs against your SOPs
  ...
  — see [Installation](installation.qmd) for SSO).
...
- [ ] Pin image tags to a released version — each release ships with its
  [validation pack](compliance.qmd).
...
The published web image currently serves the SPA through Vite's dev server
behind the reverse proxy. It works, but a production-grade static build is
open on the roadmap — track it in the repository issues.
:::
