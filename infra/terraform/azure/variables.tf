# The same variable contract as ../aws and ../digitalocean (ADR-0011).
...
variable "instance_size" {
  description = "VM size (2 vCPU / 4 GB is comfortable for the full stack)"
  type        = string
...
variable "root_volume_gb" {
  description = "OS disk size in GB (holds Postgres and the DuckLake volumes); server-side encrypted by the platform"
  type        = number
  default     = 50
}
