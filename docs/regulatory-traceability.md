...
| E6-05 | Access management: unique credentials, role-appropriate access, timely revocation | RBAC with per-study/per-site scoping; grants/revocations audited; deactivation revokes live sessions immediately (not at next timeout) | 🟢 `auth.test.ts`, `admin-users.test.ts` |
