...
// Reviewers in different time zones must read the same instant identically
// (E6(R3) 4.2.2(d)); render server UTC, never the browser's locale time.
...
export function AuditTrail({ studyId }: { studyId: string | null }) {
  ...
  // datetime-local values are in the browser's zone; the API takes ISO
  // instants, so convert (invalid/partial input is simply not applied).
  const toIso = (local: string) => {
    const date = new Date(local);
    return Number.isNaN(date.getTime()) ? undefined : date.toISOString();
  };
  const fromIso = from ? toIso(from) : undefined;
  const untilIso = to ? toIso(to) : undefined;
  const filters: AuditFilters = {
    ...(action ? { action } : {}),
    ...(entityType ? { entityType } : {}),
    ...(actor ? { actor } : {}),
    ...(fromIso ? { from: fromIso } : {}),
    ...(untilIso ? { to: untilIso } : {}),
    limit: PAGE_SIZE,
    offset,
  };
